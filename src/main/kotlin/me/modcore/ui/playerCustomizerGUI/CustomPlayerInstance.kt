...
class CustomPlayerInstance(private val playerEntry: PlayerEntry) {
    ...
    private val colorAnimations: List<ColorAnimation> = listOf(ColorAnimation(250), ColorAnimation(250), ColorAnimation(250), ColorAnimation(250), ColorAnimation(250),  ColorAnimation(250),  ColorAnimation(250))
    private val animations: List<LinearAnimation<Float>> = listOf(LinearAnimation<Float>(200), LinearAnimation<Float>(200), LinearAnimation<Float>(200), LinearAnimation<Float>(200), LinearAnimation<Float>(200),  LinearAnimation<Float>(200),  LinearAnimation<Float>(200))
    val x = 60f
    val width = 1050f
    val height = 460f
    ...
    private val textureElement: ElementTextureBox = ElementTextureBox("Texture", playerEntry, x + 612.0f - 25, 200f, 10f, 20f)
    ...
    fun mouseClicked(y: Float){
        ...
        if(
            if (!ClickGUIModule.switchType) isAreaHovered(x + toggleSwitchX+ switchOffset, y + 34 + switchHeight * 0.75f, 42f, 40f)
        ...
        if(
            if (!ClickGUIModule.switchType) isAreaHovered(x + toggleSwitchX+ switchOffset, y + 34 + switchHeight * 1.75f, 42f, 40f)
        ...
        if(
            if (!ClickGUIModule.switchType) isAreaHovered(x + toggleSwitchX+ switchOffset, y + 34 + switchHeight * 2.75f, 42f, 40f)
        ...
        if(
            if (!ClickGUIModule.switchType) isAreaHovered(x + toggleSwitchX+ switchOffset, y + 34 + switchHeight * 3.75f, 42f, 40f)
        ...
        if(
            if (!ClickGUIModule.switchType) isAreaHovered(x + toggleSwitchX+ switchOffset, y + 34 + switchHeight * 4.75f, 42f, 40f)
    ...
    fun draw(y: Float) {
        ...
        roundedRectangle(x,  y, width * 0.3, height, elementBackground, elementBackground, elementBackground, 0, 10, 0, 10, 0, 0.5)
        ...
        text("Reset", x + 645, y + 367.5, textColor.darkerIf(isAreaHovered(x + 560, y + 325, 170f, 85f)), 32f, align = TextAlign.Middle)

        roundedRectangle(x+ width - 40, y+ height - 40, 40, 40, Color.TRANSPARENT, clickGUIColor, Color.TRANSPARENT, 3, 5, 0, 0, 10, 0.5f)
        drawX(x+ width - 19, y + height - 22, 1.5f, textColor.darkerIf(isAreaHovered(x+ width - 40, y+ height - 40, 40f, 40f), 0.7f))


        drawSwitch("Toggle", x + toggleSwitchX, y + 34 + switchHeight * 0.75f, 28f, enabled = playerEntry.toggle, 0)
        drawSwitch("Hide Helmet", x + toggleSwitchX, y + 34 + switchHeight * 1.75f, 28f, enabled = playerEntry.hideHelmet, 1)
        drawSwitch("Hide Chestplate", x + toggleSwitchX, y + 34 + switchHeight * 2.75f, 28f, enabled = playerEntry.hideChestplate, 2)
        drawSwitch("Hide Leggings", x + toggleSwitchX, y + 34 + switchHeight * 3.75f, 28f, enabled = playerEntry.hideLeggings, 3)
        drawSwitch("Hide Boots", x + toggleSwitchX, y + 34 + switchHeight * 4.75f, 28f, enabled = playerEntry.hideBoots, 4)
    ...
    private fun drawSwitch(name: String, x: Float, y: Float, h: Float, enabled: Boolean, index: Int){
        ...
        if (!ClickGUIModule.switchType) {
            ...
            circle((x + switchOffset + (42f * scale)) / scale - animations[index].get(33f, 17f, !enabled), (y + (14f * scale) + 2) / scale, 6f,
